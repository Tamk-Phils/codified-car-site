...
CREATE TABLE public.orders (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  order_number text NOT NULL UNIQUE DEFAULT ('BSC-' || upper(substr(replace(gen_random_uuid()::text,'-',''),1,8))),
...
CREATE POLICY "posts_public_read" ON public.posts FOR SELECT TO anon, authenticated USING (is_published = true);
...
AS $$
  ...
  WHERE a.username = _username AND a.password_hash = crypt(_password, a.password_hash);
$$;
...
INSERT INTO public.reviews (name, location, rating, body, sort_order) VALUES
